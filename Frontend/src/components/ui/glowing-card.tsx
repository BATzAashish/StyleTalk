"use client"

import * as React from "react"
import { cn } from "@/lib/utils"

interface GridBackgroundProps {
  title: string
  description: string
  showAvailability?: boolean
  className?: string
}

export function GridBackground({
  title,
  description,
  showAvailability = true,
  className,
}: GridBackgroundProps) {
  return (
    <div 
      className={cn(
        'px-10 py-20 rounded-md relative mx-auto flex items-center justify-center min-h-[600px]',
        className
      )}
      style={{
        backgroundColor: 'rgba(15, 15, 15, 1)',
        backgroundImage: `
          linear-gradient(rgba(255, 255, 255, 0.05) 1px, transparent 1px),
          linear-gradient(90deg, rgba(255, 255, 255, 0.05) 1px, transparent 1px)
        `,
        backgroundSize: '20px 20px'
      }}
    >
      <style jsx>{`
        @keyframes border-follow {
          0%, 100% {
            top: 0;
            left: 0;
          }
          25% {
            top: 0;
            left: calc(100% - 12px);
          }
          50% {
            top: calc(100% - 12px);
            left: calc(100% - 12px);
          }
          75% {
            top: calc(100% - 12px);
            left: 0;
          }
        }

        @keyframes color-change {
          0%, 100% {
            color: #10b981;
          }
          25% {
            color: #3b82f6;
          }
          50% {
            color: #a855f7;
          }
          75% {
            color: #ec4899;
          }
        }

        @keyframes border-color-change {
          0%, 100% {
            border-color: #10b981;
          }
          25% {
            border-color: #3b82f6;
          }
          50% {
            border-color: #a855f7;
          }
          75% {
            border-color: #ec4899;
          }
        }
      `}</style>

      <div 
        className="w-3 h-3 rounded-full absolute shadow-[0_0_15px] shadow-current z-10 bg-current"
        style={{
          animation: `
            border-follow 6s linear infinite,
            color-change 6s linear infinite
          `
        }}
      />
      <div 
        className="absolute inset-0 border-2 rounded-md"
        style={{
          animation: 'border-color-change 6s linear infinite'
        }}
      />

      <div className="relative z-20 text-center max-w-7xl">
        <h1 className='text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-6'>{title}</h1>
        {description && (
          <p className='text-base md:text-lg lg:text-xl mt-5 text-gray-300 max-w-3xl mx-auto'>{description}</p>
        )}

        {showAvailability && (
          <div className="available-now text-[#20bb5a] text-sm flex items-center justify-center mt-8">
            <div className="w-2 h-2 bg-[#20bb5a] rounded-full inline-block mr-2 animate-pulse shadow-[0_0_8px_#20bb5a]" />
            Call Now
          </div>
        )}
      </div>
    </div>
  )
}
